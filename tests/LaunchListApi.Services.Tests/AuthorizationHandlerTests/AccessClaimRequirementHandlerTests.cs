...
{
    public class AccessClaimRequirementHandlerTests
    {
        // Test that the handler returns Success when the current user has the required claim.
        [Fact]
        public async void HandleAsync_UserHasClaim_ReturnsSuccessResult()
        {
            //Arrange
            AccessClaimRequirementHandler handler = new AccessClaimRequirementHandler();

            ClaimsPrincipal testUser = new ClaimsPrincipal(new ClaimsIdentity(new Claim[] { new Claim(AuthorizationClaimTypes.AuthorizationClaim, "test") }));
        ...
        [Fact]
        public async void HandleAsync_UserDoesNotHaveClaim_ReturnsFailureResult()
        {
            //Arrange
            AccessClaimRequirementHandler handler = new AccessClaimRequirementHandler();
